...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_TS__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_TS__INTC_NUMBER];
}
...
void isr_TS_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_TS__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_TS_INTC_PRIOR = (*isr_TS_INTC_PRIOR & (uint32)(~(uint32)isr_TS__INTC_PRIOR_MASK)) |
...
uint8 isr_TS_GetPriority(void)
{
    ...
    priority = (*isr_TS_INTC_PRIOR & isr_TS__INTC_PRIOR_MASK) >> priorityOffset;
